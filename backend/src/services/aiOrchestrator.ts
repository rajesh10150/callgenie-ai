...
export class AIOrchestrator {
  ...
  selectModel(context: RoutingContext): AIModel {
    ...
    switch (context.strategy) {
      ...
      case 'smart':
        if (context.taskComplexity === 'high') return 'gpt-4.1';
        if (nonEnglish) return 'gemini';
        return 'claude';
  ...
  ): Promise<{ content: string; model: string; tokensUsed: number; cost: number }> {
...
- suggestedFollowUp: recommended next action`,
  ...
  async handleObjection(
    objection: string,
    context: { industry: string; product: string; language: string }
  ): Promise<string> {
    const model = this.selectModel({
      strategy: 'fast',
...
Be empathetic but guide toward a positive outcome.`,
  ...
  getAvailableModels(): Array<{ model: AIModel; config: ModelConfig }> {
