...
router.get('/dashboard', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const [
      ...
      supabaseAdmin.from('calls').select('*', { count: 'exact', head: true }).eq('org_id', orgId),
      supabaseAdmin.from('leads').select('*', { count: 'exact', head: true }).eq('org_id', orgId),
      supabaseAdmin.from('campaigns').select('*', { count: 'exact', head: true }).eq('org_id', orgId),
      supabaseAdmin.from('leads').select('*', { count: 'exact', head: true }).eq('org_id', orgId).eq('status', 'qualified'),
      supabaseAdmin.from('calls').select('*', { count: 'exact', head: true }).eq('org_id', orgId).eq('appointment_booked', true),
    ]);
    ...
    sendError(res, 'INTERNAL_ERROR', 'Failed to fetch dashboard data', 500);
  }
});
...
router.get('/calls', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    sendError(res, 'INTERNAL_ERROR', 'Failed to fetch call analytics', 500);
  }
});
...
router.get('/leads', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    sendError(res, 'INTERNAL_ERROR', 'Failed to fetch lead analytics', 500);
  }
});
...
router.get('/campaigns', async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    sendSuccess(res, campaignStats);
  } catch {
    sendError(res, 'INTERNAL_ERROR', 'Failed to fetch campaign analytics', 500);
  }
});
